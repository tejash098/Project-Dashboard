...
const LivePreview = ({ url, title, canEdit = false }) => {
  ...
  // Admins can edit liveUrl inline on this page, so reset back to the loading
  // state whenever it changes. Adjusting state during render (rather than in an
  // effect) is React's recommended way to reset on a prop change and avoids a
  // cascading-render warning — the same idiom as the slug reset in ProjectDetail.
  ...
  if (!parsed) {
    return (
      <section className="mb-6">
        <div
          className={`${PREVIEW.FRAME} ${ROUNDED.LG} border ${BORDER.DEFAULT}
            bg-page-bg flex flex-col items-center justify-center gap-1 px-6 text-center`}
        >
  ...
  return (
    <section className="mb-6">
      ...
      <div
        className={`relative ${PREVIEW.FRAME} ${ROUNDED.LG} border ${BORDER.DEFAULT} bg-page-bg`}
      >
        {status === "blocked" ? (
